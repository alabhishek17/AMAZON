...
import { useGetProductIteamQuery,useGetProductsBySportsQuery } from "../reducer/services/productlist"; 
import { useNavigate } from "react-router-dom";
function SPOETS(){
    const navigate=useNavigate();
    const { data: sportsData, error: sportsError, isLoading: isLoadingsports } =useGetProductsBySportsQuery(
        "search?query=Sports & Outdoors&page=1&country=US&sort_by=RELEVANCE&product_condition=ALL"
      );
       
    console.log(sportsData,sportsError,isLoadingsports);
    if(isLoadingsports){
...
return(
    <div className="flex flex-wrap justify-center">
            {sportsData.data.products.map((item,asin) => (
